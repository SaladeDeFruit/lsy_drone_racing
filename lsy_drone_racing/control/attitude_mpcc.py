...
class MPCCController(Controller):
    ...
    def _build_solver(self):
        ...
        # Initialize parameters array (300 points * 3 + 300 points * 3 + 300 points = 2100)
        ocp.parameter_values = np.zeros(2100) 
        
        # External cost function
        ocp.cost.cost_type = "EXTERNAL"
        
        # IMPORTANT: Do not slice model.p here! 
        # self.pd_list, self.tp_list, and self.qc_dyn_list must be created as MX.sym 
        # attributes inside _build_dynamics_model() directly.
        ocp.model.cost_expr_ext_cost = self._build_cost_expression()
        
        # --- INPUT CONSTRAINTS (u) ---
        # Bounds on control rates: [df_cmd, dr_cmd, dp_cmd, dy_cmd, v_theta_cmd]
        ocp.constraints.lbu = np.array([-50.0, -50.0, -50.0, -50.0, -1.0]) 
        ocp.constraints.ubu = np.array([50.0, 50.0, 50.0, 50.0, 10.0])
        ocp.constraints.idxbu = np.array([0, 1, 2, 3, 4])
        
        # --- STATE CONSTRAINTS (x) ---
        # Add boundaries to prevent physically impossible states (e.g., negative thrust, infinite flip)
        thrust_min = float(self._dyn_params["thrust_min"]) * 4.0
        thrust_max = float(self._dyn_params["thrust_max"]) * 4.0
        
        # Indices for [f_cmd, r_cmd, p_cmd, y_cmd] inside your 14-state vector
        # x = [px, py, pz, vx, vy, vz, roll, pitch, yaw, f_cmd, r_cmd, p_cmd, y_cmd, theta]
        ocp.constraints.lbx = np.array([thrust_min, -1.57, -1.57, -1.57]) # -90 degrees max
        ocp.constraints.ubx = np.array([thrust_max, 1.57, 1.57, 1.57])    # +90 degrees max
    ...
    def render_callback(self, sim: Sim):
        ...
        Redessinée à chaque frame : reflète automatiquement les re-planifications (la spline
        est régénérée quand une porte/obstacle est découvert).
        """
