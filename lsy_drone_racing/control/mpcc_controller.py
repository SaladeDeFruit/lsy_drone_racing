...
@dataclass
class MPCCConfig:
    """Configuration for MPCC controller - Tuned for Crazyflie cf21B_500"""
    # MPC Horizon
    # Horizon légèrement réduit : le Crazyflie a une dynamique rapide, un dt de 0.02s (0.6/30) est idéal.
    N_horizon: int = 30                    
    T_horizon: float = 0.6                 
    
    # Arc-length model
    model_arc_step: float = 0.05            
    model_traj_length: float = 12.0         # Réduit car le drone voit "moins loin" à cause de sa vitesse modérée
    
    # Cost function weights
    # Augmentation des poids de contour pour forcer le drone léger à rester sur sa ligne (très sensible au drift)
    q_lag: float = 80.0                    
    q_lag_peak: float = 500.0              
    q_contour: float = 200.0               # Augmenté (était 120.0) - Priorité à la précision
    q_contour_peak: float = 800.0          # Augmenté (était 700.0) aux portes
    q_attitude: float = 3.0                # Augmenté (était 1.0) - Évite les angles d'inclinaison extrêmes
    
    # Control smoothness
    # Pénalités augmentées pour le roulis et le tangage. Le Crazyflie est très nerveux,
    # des commandes trop brusques entraînent des oscillations (wobble).
    r_thrust: float = 0.2                  
    r_roll: float = 0.6                    # Augmenté (était 0.3)
    r_pitch: float = 0.6                   # Augmenté (était 0.3)
    r_yaw: float = 0.50                    
    
    # Speed incentive
    # Récompense de vitesse diminuée. Trop de mu_speed forcera le drone à piquer du nez et chuter.
    mu_speed: float = 5.5                   # Diminué (était 10.0)
    w_speed_gate: float = 12.0              # Freinage plus fort près des portes (était 9.0)
    ...
    # Vitesse maximale physiquement plus réaliste pour un vol indoor stable avec ce modèle
    vel_bounds: tuple = (-1.0, 3.2)         # Vitesse max réduite à 3.2 m/s (était 5.0 m/s)
...
class MPCCController(Controller):
    ...
    def _plan_trajectory(self, obs: dict[str, NDArray[np.floating]]):
        """Plan or replan the trajectory from current position for remaining gates."""
        print(f"[MPCC] Planning trajectory at T={self._step_count / self._ctrl_freq:.2f}s")
    
        g0 = max(0, int(obs.get('target_gate', 0)))
        if int(obs.get('target_gate', 0)) == -1:
            return  # Fin de parcours, on garde l'ancien tracé
            
        obs_planning = obs.copy()
        obs_planning['pos'] = obs['pos'].copy()  # On part de la position courante du drone
        
        # Ne conserver que les portes et obstacles restants
        obs_planning['gates_pos'] = obs['gates_pos'][g0:]
        obs_planning['gates_quat'] = obs['gates_quat'][g0:]
        ...
        # Cache for cost computation (Crucial: ne garder que les portes restantes)
        self._cached_gate_centers = obs_planning['gates_pos'].copy()
        self._cached_obstacles = obs.get("obstacles_pos", []).copy()
    ...
    def _build_solver(self):
        ...
        ocp.solver_options.nlp_solver_type = "SQP"  # Changé de SQP_RTI à SQP complet pour gérer les replans
        ocp.solver_options.tol = 1e-5
        ocp.solver_options.qp_solver_cond_N = self.N
        ocp.solver_options.qp_solver_warm_start = 1
        ocp.solver_options.qp_solver_iter_max = 20
        ocp.solver_options.nlp_solver_max_iter = 10  # Permet de converger en quelques itérations lors des sauts de spline
    ...
    def compute_control(
        ...
        if self._detect_environment_change(obs):
            ...
            # 1. Sauvegarder l'ancienne valeur de theta avant réinitialisation
            old_theta = getattr(self, 'last_theta', 0.0)
            
            # 2. Planifier le nouveau tracé depuis la position courante pour les portes restantes
            self._plan_trajectory(obs)
            
            # 3. Réancrer la progression au début (0.0) de la nouvelle spline
            self.last_theta = 0.0

            # 4. --- WARM-START GLISSÉ ---
            if hasattr(self, "_x_warm") and self._x_warm is not None:
                for i in range(len(self._x_warm)):
                    # L'état theta est le dernier élément (index 14) du vecteur d'état.
                    # On soustrait l'ancien décalage pour recaler l'horizon temporel sur 0.0
                    self._x_warm[i][-1] -= old_theta
                    self._x_warm[i][-1] = max(0.0, self._x_warm[i][-1])
        ...
        # Check termination conditions (avec les diagnostics de sécurité d'origine remis en place)
    ...
    def render_callback(self, sim: Sim):
        """Affiche EN DIRECT, en vert, la trajectoire de référence que le MPCC suit.
        """
